...
#include "pvs_countpairs.h" //function proto-type
...
results_pvs_countpairs * countpairs_pvs(const int64_t ND1, const DOUBLE * const X1, const DOUBLE * const Y1, const DOUBLE  * const Z1,const DOUBLE *VX1, const DOUBLE *VY1, const DOUBLE *VZ1,
                                ...
                                const char *binfile)
{
    ...
    cellarray_nvec_pvs *lattice1 = gridlink_nvec_pvs(ND1, X1, Y1, Z1, VX1, VY1, VZ1, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &nmesh_x, &nmesh_y, &nmesh_z);
    if(nmesh_x <= 10 && nmesh_y <= 10 && nmesh_z <= 10) {
        ...
        free_cellarray_nvec_pvs(lattice1, totncells);
        lattice1 = gridlink_nvec_pvs(ND1, X1, Y1, Z1, VX1, VY1, VZ1, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &nmesh_x, &nmesh_y, &nmesh_z);
    }


    cellarray_nvec_pvs *lattice2 = NULL;
    if(autocorr==0) {
        int ngrid2_x=0,ngrid2_y=0,ngrid2_z=0;
        lattice2 = gridlink_nvec_pvs(ND2, X2, Y2, Z2, VX2, VY2, VZ2, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &ngrid2_x, &ngrid2_y, &ngrid2_z);
...
#endif
    DOUBLE **all_vpavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,nrpbin);
    DOUBLE **all_spavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,nrpbin);
    DOUBLE **all_vtavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,nrpbin);
    DOUBLE **all_stavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,nrpbin);
#endif//USE_OMP

    DOUBLE sqr_rpmax=rupp_sqr[nrpbin-1];
    DOUBLE sqr_rpmin=rupp_sqr[0];

// AVX NOT IMPLEMENTED YET
#if defined(USE_AVX) && defined(__AVX__)
...
#pragma omp atomic
            ...
            cellarray_nvec_pvs *first = &(lattice1[index1]);
            ...
            for(int iix=-bin_refine_factor;iix<=bin_refine_factor;iix++){
                ...
                for(int iiy=-bin_refine_factor;iiy<=bin_refine_factor;iiy++){
                    ...
                    for(int iiz=-bin_refine_factor;iiz<=bin_refine_factor;iiz++){
                        ...
                        const cellarray_nvec_pvs *second = &(lattice2[index2]);
                        DOUBLE *x1 = first->pos;
                        DOUBLE *y1 = (first->pos) + NVEC;
                        DOUBLE *z1 = (first->pos) + 2*NVEC;
                        DOUBLE *vx1 =  first->vel;
                        DOUBLE *vy1 = (first->vel) + NVEC;
                        DOUBLE *vz1 = (first->vel) + 2*NVEC;

                        DOUBLE *x2 = second->pos;
                        DOUBLE *y2 = (second->pos) + NVEC;
                        DOUBLE *z2 = (second->pos) + 2*NVEC;
                        DOUBLE *vx2 =  second->vel;
                        DOUBLE *vy2 = (second->vel) + NVEC;
                        DOUBLE *vz2 = (second->vel) + 2*NVEC;
                        ...
                        for(int64_t i=0;i<first->nelements;i+=NVEC) {
                            ...
                            for(int ii=0;ii<block_size1;ii++) {
                                ...
                                DOUBLE y1pos = y1[ii];
                                DOUBLE z1pos = z1[ii];
                                DOUBLE vx1pos = vx1[ii];
                                DOUBLE vy1pos = vy1[ii];
                                DOUBLE vz1pos = vz1[ii];
...
#pragma unroll(NVEC)
...
#endif
            all_vpavg[tid][j] = vpavg[j];
            all_spavg[tid][j] = spavg[j];
            all_vtavg[tid][j] = vtavg[j];
            all_stavg[tid][j] = stavg[j];
        }
...
#endif
            vpavg[j] += all_vpavg[i][j];
            spavg[j] += all_spavg[i][j];
            vtavg[j] += all_vtavg[i][j];
            stavg[j] += all_stavg[i][j];
        }
    }
    ...
    free_cellarray_nvec_pvs(lattice1, totncells);
    if(autocorr == 0) {
        free_cellarray_nvec_pvs(lattice2, totncells);
    }
    free(rupp);
...
#endif
    matrix_free((void **) all_vpavg, numthreads);
    matrix_free((void **) all_spavg, numthreads);
    matrix_free((void **) all_vtavg, numthreads);
    matrix_free((void **) all_stavg, numthreads);
