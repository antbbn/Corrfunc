...
/* #endif */
   ...
   ************************/
	...
	cellarray_pvs *lattice1 = gridlink_pvs(ND1, X1, Y1, Z1, VX1, VY1, VZ1, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, pimax, bin_refine_factor, bin_refine_factor, zbin_refine_factor, &nmesh_x, &nmesh_y, &nmesh_z);
	cellarray_pvs *lattice2 = NULL;
	if(autocorr==0) {
		int ngrid2_x=0,ngrid2_y=0,ngrid2_z=0;
		lattice2 = gridlink_pvs(ND2, X2, Y2, Z2, VX2, VY2, VZ2, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, pimax, bin_refine_factor, bin_refine_factor, zbin_refine_factor, &ngrid2_x, &ngrid2_y, &ngrid2_z);
	...
	DOUBLE **all_vpavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,totnbins);
	DOUBLE **all_spavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,totnbins);
	DOUBLE **all_vtavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,totnbins);
	DOUBLE **all_stavg = (DOUBLE **) matrix_calloc(sizeof(DOUBLE),numthreads,totnbins);
#ifdef OUTPUT_RPAVG
			...
			for(int iix=-bin_refine_factor;iix<=bin_refine_factor;iix++){
				...
				for(int iiy=-bin_refine_factor;iiy<=bin_refine_factor;iiy++){
					...
					for(int iiz=-zbin_refine_factor;iiz<=zbin_refine_factor;iiz++){
						...
						const cellarray_pvs *second = &(lattice2[index2]);
						const cellarray_pvs *first  = &(lattice1[index1]);
						...
						for(int64_t i=0;i<first->nelements;i+=NVEC){
							...
							for(int ii=0;ii<block_size1;ii++) {
								...
								DOUBLE y1pos = y1[ii];
								DOUBLE z1pos = z1[ii];
								DOUBLE vx1pos = vx1[ii]; //name does not make much sense
								...
								for(int64_t j=0;j<second->nelements;j+=NVEC) {
									...
									for(int jj=0;jj<block_size2;jj++) {
										...
										const DOUBLE dvx =      localvx2[jj]-vx1pos;
										const DOUBLE dvy =      localvy2[jj]-vy1pos;
										const DOUBLE dvz =      localvz2[jj]-vz1pos;
...
#pragma unroll(NVEC)
		...
		for(int i=0;i<totnbins;i++) {
			...
			all_vpavg[tid][i] = vpavg[i];
			all_spavg[tid][i] = spavg[i];
			all_vtavg[tid][i] = vtavg[i];
			all_stavg[tid][i] = stavg[i];
#ifdef OUTPUT_RPAVG			
