...
int main(int argc, char *argv[])
{
  ...
  if (autocorr==0) {
    /*---Read-data2-file----------------------------------*/
    ND1=read_positions(file2,fileformat2,sizeof(DOUBLE), 6, &x2, &y2, &z2,&vx2, &vy2, &vz2);
    gettimeofday(&t1,NULL);
    read_time += ADD_DIFF_TIME(t0,t1);
  ...
  /*---Count-pairs--------------------------------------*/
  gettimeofday(&t0,NULL);
  results_pvs_countpairs_rp_pi *results = bruteforce_pvs_countpairs_rp_pi(ND1,x1,y1,z1,vx1,vy1,vz1,
