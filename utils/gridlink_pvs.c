...
#include "defs.h"
#include "gridlink_pvs.h"
#include "utils.h"
#include "function_precision.h"
#include "cellarray_pvs.h"
...
cellarray_pvs * gridlink_pvs(const int64_t np,
										 ...
										 int *nlattice_z)
{
...
#endif	
  lattice    = (cellarray_pvs *) my_malloc(sizeof(cellarray_pvs), totncells);
  ...
  for (int64_t index=0;index<totncells;index++) {
		...
		lattice[index].vx = my_malloc(memsize,expected_n);//This allocates extra and is wasteful
		lattice[index].vy = my_malloc(memsize,expected_n);//This allocates extra and is wasteful
		lattice[index].vz = my_malloc(memsize,expected_n);//This allocates extra and is wasteful
  ...
  for (int64_t i=0;i<np;i++)  {
    ...
    if(lattice[index].nelements == nallocated[index]) {
      ...
      while(expected_n <= nallocated[index] || ((expected_n % NVEC) != 0))
			...
			lattice[index].vx = my_realloc(lattice[index].vx ,memsize,expected_n,"lattice.vx");
			lattice[index].vy = my_realloc(lattice[index].vy ,memsize,expected_n,"lattice.vy");
			lattice[index].vz = my_realloc(lattice[index].vz ,memsize,expected_n,"lattice.vz");
			nallocated[index] = expected_n;
    }
    assert(lattice[index].nelements < nallocated[index] && "Ensuring that number of particles in a cell doesn't corrupt memory");
		...
		lattice[index].vx[ipos] = vx[i];
		lattice[index].vy[ipos] = vy[i];
		lattice[index].vz[ipos] = vz[i];
